...
const publishAVideo = asyncHandler(async (req, res) => {
    // Try to get from req.body first (normal case), then fall back to query params (multipart case)
    ...
    if (!title?.trim() || !description?.trim() || !duration?.trim()) {
        throw new ApiError(400, "title, description, and duration are all required");
    }
    ...
    const video = await Video.create({
        title,
        description,
        duration: videoFile.duration || duration,
